...
it('401s when auth returns 401 (bad/expired token)', function () {
...
it('403s a deactivated user with a still-valid token', function () {
    $headers = actingAsRole('admin_hrd', null, active: false);
    $this->getJson('/_probe', $headers)->assertStatus(403);
});

it('passes a valid active identity through', function () {
    $headers = actingAsRole('admin_hrd');
    $this->getJson('/_probe', $headers)->assertOk();
});
